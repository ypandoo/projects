...
public class SubscribeController {
	...
	public ResultDTO updateDimissionByCsrid(@RequestParam("csrId") String csrId, @RequestParam("isDimission") boolean isDimission) {
		...
		if(isDimission == true){
			record.setDimissionDate(new Date());
		}else{
			record.setDimissionDate(null);
		}
		
		record.setDimissionAmt(BigDecimal.valueOf(0));
	...
	@RequestMapping(value = "/updateDimissionByUid", method = RequestMethod.POST)
	@ResponseBody
	public ResultDTO updateDimissionByUid(@RequestParam("uid") String uid, @RequestParam("isDimission") boolean isDimission) {
		ResultDTO result = new ResultDTO();
		...
		int res = completeSubscribeRecordService.updateDimissionByUid(record);
		if(res > 0){
			result.setSuccess(true);
		}else{
			result.setSuccess(false);
	 ...
	 * @return
	 */
	@RequestMapping("/callDimissionExport")
	public @ResponseBody ResultDTO callDimissionExport(HttpSession session, HttpServletResponse response,  @RequestParam("projectId") String projectId) {
		ResultDTO result = new ResultDTO();
		...
		param.setPageSize(999);
		param.setProjectId(projectId);
		param.setDimission(true);
		List<DimissionInfoDTO> data = completeSubscribeRecordService.queryDimissionListByExport(param);

		ServletContext application = session.getServletContext();
		String serverRealPath = application.getRealPath("/");
		String srcFilePath = serverRealPath + "templet//Dimission.xlsx";
		String newFile = "d://export//" + "离职人员-" + BaseUtil.formatDate(new Date(), "yyyyMMddHHmmss") + ".xlsx";
		ExcelUtil.writeDimissionExcel(srcFilePath, newFile, data, response);
		result.setDataDto(data);
		result.setSuccess(true);
		return result;
	}
